        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-1">Daftarkan pelayanan gereja baru</p>
                </div>
            ...
            <form method="POST" action="{{ route('ministries.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="category" name="category" required
                                ...
                                <option value="pemuridan" {{ old('category') == 'pemuridan' ? 'selected' : '' }}>
                                    Pemuridan</option>
                                <option value="paduan suara" {{ old('category') == 'paduan suara' ? 'selected' : '' }}>
                                    Paduan Suara</option>
                                <option value="sekolah minggu"
                                    {{ old('category') == 'sekolah minggu' ? 'selected' : '' }}>Sekolah Minggu</option>
                                <option value="multimedia" {{ old('category') == 'multimedia' ? 'selected' : '' }}>
                                    Multimedia</option>
                                <option value="kebersihan" {{ old('category') == 'kebersihan' ? 'selected' : '' }}>
                                    Kebersihan</option>
                                <option value="keamanan" {{ old('category') == 'keamanan' ? 'selected' : '' }}>Keamanan
                                </option>
                            ...
                            <select id="leader_id" name="leader_id"
                                ...
                                @foreach ($members as $member)
                                    <option value="{{ $member->id }}"
                                        {{ old('leader_id') == $member->id ? 'selected' : '' }}>
                                        {{ $member->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                        ...
                        placeholder="Masukkan deskripsi pelayanan">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
