<x-layouts.app>
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-1">Tambah data pengeluaran baru</p>
                </div>
        ...
        <!-- Form -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('finances.store-expense') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('category') border-red-500 @enderror">
                            ...
                            <option value="pemeliharaan" {{ old('category') == 'pemeliharaan' ? 'selected' : '' }}>Pemeliharaan</option>
                            <option value="perbaikan" {{ old('category') == 'perbaikan' ? 'selected' : '' }}>Perbaikan</option>
                            <option value="pembelian" {{ old('category') == 'pembelian' ? 'selected' : '' }}>Pembelian</option>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3" 
                        ...
                        placeholder="Tambahkan catatan pengeluaran (opsional)">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
