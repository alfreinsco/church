    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Tambah Persembahan</h1>
                    <p class="text-gray-600 mt-1">Tambah data persembahan baru</p>
                </div>
        ...
        <!-- Form -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('finances.store-offering') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('type') border-red-500 @enderror">
                            ...
                            <option value="syukur" {{ old('type') == 'syukur' ? 'selected' : '' }}>Syukur</option>
                            <option value="misi" {{ old('type') == 'misi' ? 'selected' : '' }}>Misi</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                        ...
                        placeholder="Tambahkan deskripsi persembahan (opsional)">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
